...
-- 01. Секция для ключа секционирования определяется путем применения хеширования к значению ключа секционирования
-- 02. Кол-во секций желательно равно степени числа 2. (например 4, 8, 16)
-- 03. Значение столбца для хеш секционирования должна быть равной степени 2. Потому что так данные распределяются более равномерно по секциям. (Кайт).
-- 04. В основном применяется для разделения потоков при записи, например, промежуточный этап расчетов (запись из многих потоков, чтение в параллели)
